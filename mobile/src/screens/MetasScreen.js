...
import { CUENTAS, formatearNumero, calcularSaldosPorCuenta, generarIdMeta } from '../lib/finance';
...
export default function MetasScreen() {
  ...
  function crearMeta() {
    ...
    replaceState((s) => ({
      ...s,
      metas: [...(s.metas || []), { id: metaId, nombre: nombre.trim(), objetivo: obj, plazo: pl || null }],
      contribucionesMetas: [
          ...
          metas.map((meta) => {
            ...
            return (
              <View key={meta.id} style={styles.metaRow}>
                <Text style={styles.metaTit}>{meta.nombre}</Text>
                <Text style={typography.small}>
...
const styles = StyleSheet.create({
  ...
  metaTit: { color: colors.text, fontWeight: '700', fontSize: 16, marginBottom: 4, flexShrink: 1 },
  barBg: {
    height: 7,
